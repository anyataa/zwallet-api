...
public interface TransactionRepository extends JpaRepository<TransactionEntity, Integer> {
    ...
    @Query(value = "SELECT * FROM transaction_table WHERE from_account_id = ?1 AND transaction_timestamp = current_date() OR to_account_id = ?1 AND transaction_timestamp = current_date() ORDER BY transaction_id DESC LIMIT 2", nativeQuery = true)
    List<TransactionEntity> findToday2Transaction(AccountEntity accountId);

    @Query(value = "SELECT * FROM transaction_table WHERE from_account_id = ?1 AND transaction_timestamp BETWEEN curdate()-7 AND curdate() -1 OR to_account_id = ?1 AND transaction_timestamp BETWEEN curdate()-7 AND curdate() -1 ORDER BY transaction_id DESC LIMIT 2", nativeQuery = true)
